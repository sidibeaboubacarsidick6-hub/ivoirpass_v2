...
class NotificationService:
    ...
    def send_ticket_confirmation(order):
        """Envoie la confirmation de commande avec les billets (utilisateur connecté)."""
    ...
    # Alias pour compatibilité avec les signaux existants
    @classmethod
    ...
    def guest_tickets_confirmed(cls, order):
        """
        Envoie les billets PDF à un acheteur SANS compte.
        Le PDF est joint directement à l'email + un lien de téléchargement est fourni.
        """
        from apps.tickets.models import GuestTicket
        from apps.tickets.utils import generate_guest_ticket_pdf

        tickets = GuestTicket.objects.filter(
            order_item__order=order
        ).select_related('order_item__ticket_type__event')

        if not tickets.exists():
            logger.warning(f"Aucun ticket trouvé pour la commande {order.order_number}")
            return False

        base_url = settings.PAYDUNYA_BASE_URL

        # Génère les PDF en pièces jointes
        attachments = []
        for ticket in tickets:
            try:
                pdf_bytes = generate_guest_ticket_pdf(ticket)
                attachments.append((
                    f"billet-ivoirpass-{ticket.ticket_number}.pdf",
                    pdf_bytes,
                    'application/pdf'
                ))
            except Exception as e:
        ...
        tickets_with_links = []
        for ticket in tickets:
            tickets_with_links.append({
                'ticket':       ticket,
                'download_url': f"{base_url}/billets/guest/billet/{ticket.ticket_number}/pdf/",
            })

        first_event_title = tickets.first().event.title if tickets.exists() else ''
        subject = f"Vos billets — {order.order_number} — {first_event_title}"
        ...
        context = {
            ...
            'support_email':      settings.IVOIRPASS.get('CONTACT_EMAIL', 'infos@mks-soft-technologies.com'),
            'year':               __import__('django.utils.timezone', fromlist=['now']).now().year,
        }
        ...
        try:
            email.send()
            logger.info(f"Email guest tickets envoyé à {order.email}")
            return True
        except Exception as e:
    ...
    def welcome(cls, user):
        """Email de bienvenue après inscription organisateur."""
        context = {
            'user': user,
            'platform_name': 'IvoirPass',
            'platform_url': settings.PAYDUNYA_BASE_URL,
        ...
        try:
            ...
            logger.error(f"Template welcome introuvable: {e}")
            return False

        email = EmailMultiAlternatives(
            subject="Bienvenue sur IvoirPass !",
            body=plain_message,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[user.email],
        )
    ...
    def store_order_confirmed(cls, order):
        """Email de confirmation commande boutique — numérique OU physique, jamais les deux."""
        ...
        if order.product.is_digital:
            ...
            links_with_urls = [
                {'link': l, 'url': f"{base_url}/boutique/telecharger/{l.token}/"}
        ...
        try:
            ...
            logger.error(f"Template {template_name} introuvable: {e}")
            return False
        ...
        try:
            email.send()
            logger.info(f"Email store order confirmé envoyé à {order.buyer.email}")
            return True
        except Exception as e:
            logger.error(f"Erreur email store : {e}")
            return False

    @classmethod
    def guest_store_order_confirmed(cls, order):
        """
        Email confirmation boutique invité — numérique ou physique.
        Envoie les liens de téléchargement pour les produits numériques.
        """
        ...
        if order.product.is_digital:
            ...
            context = {
                ...
                'support_email': settings.IVOIRPASS.get('CONTACT_EMAIL', 'infos@mks-soft-technologies.com'),
                'buyer_name': order.buyer_name,
                'tracking_number': order.tracking_number or 'En attente de numéro de suivi',
            }
        ...
        # Si produit numérique, attacher un résumé des liens dans un fichier texte
        if order.product.is_digital and download_links.exists():
            try:
                links_text = "Liens de téléchargement IvoirPass\n"
                links_text += "=" * 40 + "\n\n"
                for idx, item in enumerate(links_with_urls, 1):
                    links_text += f"{idx}. {item['link'].product.name}\n"
                    links_text += f"   Token: {item['link'].token}\n"
                    links_text += f"   URL: {item['url']}\n"
                    links_text += f"   Expire le: {item['link'].expires_at.strftime('%d/%m/%Y à %H:%M')}\n"
                    links_text += f"   Téléchargements restants: {item['link'].downloads_remaining}\n\n"
                links_text += "=" * 40 + "\n"
                links_text += f"Commandé le: {order.created_at.strftime('%d/%m/%Y à %H:%M')}\n"
                links_text += f"Numéro de commande: {order.order_number}\n"
                
                email.attach(
                    f"liens-telechargement-{order.order_number}.txt",
                    links_text,
                    'text/plain'
                )
            except Exception as e:
                logger.error(f"Erreur création fichier liens: {e}")

        try:
            email.send()
            logger.info(f"Email guest store confirmé envoyé à {order.email}")
            return True
        except Exception as e:
            logger.error(f"Erreur email guest store : {e}")
            return False

    @classmethod
    def withdrawal_received(cls, withdrawal_request):
        """Confirme la réception d'une demande de reversement."""
        ...
        try:
            ...
            logger.error(f"Template withdrawal_received introuvable: {e}")
            return False

        email = EmailMultiAlternatives(
            subject=f"Demande de reversement reçue — {withdrawal_request.reference}",
            body=plain_message,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[user.email],
        )
        ...
        try:
            email.send()
            logger.info(f"Email withdrawal reçu envoyé à {user.email}")
            return True
        except Exception as e:
            logger.error(f"Erreur email withdrawal : {e}")
            return False

    @classmethod
    def withdrawal_processed(cls, withdrawal_request):
        """Notifie que le reversement a été effectué."""
        wallet = withdrawal_request.wallet
        user = wallet.organizer
        year = __import__('django.utils.timezone', fromlist=['now']).now().year
        ...
        try:
            ...
            logger.error(f"Template withdrawal_processed introuvable: {e}")
            return False

        email = EmailMultiAlternatives(
            subject=f"Reversement effectué — {withdrawal_request.reference}",
            body=plain_message,
            from_email=settings.DEFAULT_FROM_EMAIL,
            to=[user.email],
        )
        ...
        try:
            email.send()
            logger.info(f"Email withdrawal processed envoyé à {user.email}")
            return True
        except Exception as e:
            logger.error(f"Erreur email withdrawal processed : {e}")
            return False
    ...
    def event_reminder(cls, ticket):
        ...
        try:
            ...
            logger.error(f"Template event_reminder introuvable: {e}")
            return False
        ...
        try:
            email.send()
            logger.info(f"Email reminder envoyé à {user.email}")
            return True
        except Exception as e:
            logger.error(f"Erreur email reminder : {e}")
            return False
    ...
    def event_cancelled(cls, ticket):
        ...
        try:
            ...
            logger.error(f"Template event_cancelled introuvable: {e}")
            return False
        ...
        try:
            email.send()
            logger.info(f"Email cancelled envoyé à {user.email}")
            return True
        except Exception as e:
            logger.error(f"Erreur email cancelled : {e}")
            return False

    @classmethod
    def send_download_link_email(cls, order):
        """
        Envoie les liens de téléchargement pour un produit numérique.
        Alias pour store_order_confirmed.
        """
...
def notify_seller_new_order(cls, order, is_guest=False):
    """
    Notifie le vendeur qu'une nouvelle commande physique
    nécessite une préparation/livraison.
    Fonctionne pour les commandes avec ou sans compte.
    """
    # Ne notifie que pour les produits physiques (bundle inclus)
    ...
    email = EmailMultiAlternatives(
        subject=f"📦 Nouvelle commande à livrer — {order.order_number}",
        body=plain_message,
        from_email=settings.DEFAULT_FROM_EMAIL,
        to=[seller.email],
    )
    ...
    try:
        email.send()
        logger.info(f"Notification vendeur envoyée à {seller.email} pour {order.order_number}")
        return True
    except Exception as e:
